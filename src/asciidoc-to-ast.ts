...
export class Converter {
	...
	constructor(text: string) {
		...
		for (const line of this.lines) {
			const lastChar = this.chars.at(-1) ?? 0
			this.chars.push(lastChar + line.length + 1)
		}
	}
	...
	convertDocument(
		...
		const header = elem.getHeader() as unknown as Asciidoctor.Section | null
		if (
			header &&
			typeof header.getTitle === 'function' &&
			typeof header.getLevel === 'function'
		) {
			children = [
	...
	convertHeader(elem: Asciidoctor.Section, cursor: LineCursor): AnyTxtNode[] {
		const raw = (elem as unknown as { title: string }).title // .title returns raw string.
	...
	convertParagraph(elem: Asciidoctor.Block, cursor: LineCursor): AnyTxtNode[] {
		if (elem.getBlocks().length > 0) {
			...
			const node: AnyTxtNode = {
				...
				children: children as AnyTxtNode[],
				raw,
				...this.locAndRangeFrom(children),
			} as AnyTxtNode

			if (elem.hasTitle()) {
				const title = elem.getTitle()
				if (title) {
					;(node as unknown as { title: string }).title = title
				}
			}
		...
		if (elem.hasText()) {
			children = [
				...this.createParagraph(
					(elem as unknown as { text: string }).text,
					elem.getText(),
					cursor,
				),
				...children,
	...
	convertTableRow(
		...
		for (const cell of row) {
			...
			if (children.length > 0) {
				...
				if (sibling?.loc && sibling.loc.end.line === cellLine.min) {
	...
	convertListing(elem: Asciidoctor.Block, cursor: LineCursor): AnyTxtNode[] {
		...
		if (attributes?.language) {
			;(node as unknown as { lang: string }).lang = attributes.language
		}
	...
	findLocation(
		...
		for (let i = cursor.min; i + lines.length - 1 <= cursor.max; i++) {
			found = true
			const offset = 0 // see "comment in paragraph" test case.
			const startIdx = cursor.startIdx || 0 // index into the line to begin the search

			for (let j = 0; j < lines.length; j++) {
				const line = this.lines[i + j - 1 + offset]
				if (line === undefined) {
					found = false
					break
				}
